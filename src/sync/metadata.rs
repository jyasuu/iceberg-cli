...
pub async fn read_watermark(
    ...
    let table = catalog
        .load_table(ident)
        .await
        .with_context(|| format!("load_table for watermark read: {ident}"))?;
...
pub fn build_metadata_updates(
    ...
    updates.insert(KEY_LAST_RUN_AT.to_string(), Utc::now().to_rfc3339());
    updates.insert(KEY_LAST_RUN_ROWS.to_string(), rows_written.to_string());
